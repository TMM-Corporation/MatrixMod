...
var Structure = function (name, alerted) {
	...
	this.set = function (x, y, z, rotate, progressively, time, decreaseItems) {
		...
		new java.lang.Thread(function () {
			for (var i = 0; i < structure.length; i++) {
				...
				if (decreaseItems)
					for (let slot, i = 0; i < 36; i++) {
						slot = Player.getInventorySlot(i)
						if (slot.id == id && slot.data == data && slot.count >= 1) {
				...
				if(block.id != id && block.data !=data)
				World.destroyBlock(x + dx, y + dy, z + dz, true)
	...
	this.destroy = function (x, y, z, rotates, progressively, time, increaseItems) {
		if (rotates === undefined) rotates = Structure.ROTATE_NONE
		if (progressively === undefined) progressively = Structure.NOT_PROGRESSIVELY
		...
		if (index != -1) {
			...
			new java.lang.Thread(function () {
				let dropList = []
				destroying: for (var i = 0; i < structure.length; i++) {
					var block = structure[i]
					...
					if (increaseItems)
						for (let slot, i = 0; i < 36; i++) {
							slot = Player.getInventorySlot(i)
							if (slot.id == block.id && slot.data == block.data && slot.count < 64) {
								slot.count++; break
							} else if (slot.id == 0 || !slot.id) {
								slot.id = block.id; slot.data = block.data; slot.count = 1
							}else break destroying
						}
					else World.setBlock(x + dx, y + dy, z + dz, 0)
					if (progressively)
